...
const updateResultsList = () => {
 resultList.innerHTML = "";
  ...
  const qty = +ingredientQuantity.value;
  const currentUnit = unitToConvert.value;
  const servings = +numberOfServings.value;

  for (const u of units) {
    if (u !== currentUnit) {
      const num = processIngredient(qty, currentUnit, u, servings);
      const li = document.createElement("li");
      li.textContent = `${name}: ${num} ${u}`;
